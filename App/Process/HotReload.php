...
class HotReload extends AbstractProcess
{
    ...
    public function run(Process $process)
    {
        ...
        if (extension_loaded('inotify') && !$disableInotify) {
            ...
            echo "dev server hot reload start : use inotify\n";
        } else {
            // 扩展不可用时 进行暴力扫描
            ...
            echo "dev server hot reload start : use timer tick comparison\n";
        }
    }
    /**
     * 扫描文件变更
    ...
    private function runComparison()
    {
        ...
        $filesView = File::scanDirectory(EASYSWOOLE_ROOT . '/Views');
        $files["files"] = array_merge($files["files"], $filesView["files"]);
